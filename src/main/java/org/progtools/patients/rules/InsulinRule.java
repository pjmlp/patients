/* InsulinRule.java - Defines the rules for insulin.
...
/**
 * Cures fever, changing the patient state to healthy.
 */
public class InsulinRule implements  TreatmentRule {
    /**
     * @see TreatmentRule
     */
    @Override
    public Patient applyTreatment(Patient patient, Set<Drugs> drugs) {
        if (drugs.contains(Drugs.Insulin)) {
            if (patient.getState() == States.Healthy && drugs.contains(Drugs.Antibiotic)) {
                patient.setState(States.Fever);
            }
        }
