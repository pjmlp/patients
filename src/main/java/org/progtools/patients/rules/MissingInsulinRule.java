/* MissingInsulinRule.java - Defines the rules for when insulin is missing.
...
/**
 * Kills diabetic patients if insulin isn't present.
 */
public class MissingInsulinRule implements  TreatmentRule {
    /**
     * @see TreatmentRule
     */
    @Override
    public Patient applyTreatment(Patient patient, Set<Drugs> drugs) {
        if (patient.getState() == States.Diabetes && !drugs.contains(Drugs.Insulin)) {
